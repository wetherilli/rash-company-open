...
const UNIQUE_SKILLS = {
  cha_minjun: {
    /* v — 「준 피해」가 아니라 «자신 최대 체력»의 v%(사용자 지침 2026-09-02
     * — 준 피해 기준이던 것을 자신 체력 기준으로 바꿈). 다 차서 흘러넘친
     * 만큼은 다음 차례 공격력에 그대로 얹힙니다(engine.js swing() 의
     * _vampBonus). 12단계는 100% — 죽기 직전이면 통째로 채우고도 남는다. */
    name: "흡혈", kind: "active",
    tiers: { 1: 20, 4: 50, 8: 75, 12: 100 },
    desc: v => "강화 공격, 자신 체력의 " + v + "% 회복 · 넘친 만큼 다음 공격력 강화 · 같은 적에게 한 번"
  },
  kim_haju: {
    /* v — 받은 피해(방어·교정 등으로 경감되기 «전» 값)에 곱하는 배율(사용자
     * 지침 2026-09-02 — 원래는 자기 공격력 기반의 확정 치명타였음). */
    name: "반격", kind: "active",
    tiers: { 1: 1.3, 4: 1.5, 8: 1.8, 12: 2.2 },
    desc: v => "맞고 살아 있으면, 받은 피해(경감 전) × " + v + "로 반격 · 같은 적에게 한 번"
  },
  park_suo: {
    name: "회피", kind: "active",
    tiers: { 1: 1.3, 4: 1.5, 8: 1.8, 12: 2.2 },
  ...
  song_hamin: {
    /* 12단계 기준 80% 경감이 되도록 전체를 끌어올림(사용자 지침 2026-09-02
     * — 원래 3/5/8/12였던 것을 같은 모양으로 키움) */
    name: "도발", kind: "active",
    tiers: { 1: 20, 4: 35, 8: 55, 12: 80 },
    desc: v => "노려지는 대상을 자신으로, 받는 피해 " + v + "% 감소 · 같은 적에게 한 번"
  },
  chu_minsu: {
    /* 12단계 기준 80% 경감이 되도록 전체를 끌어올림(사용자 지침 2026-09-02
     * — 원래 7/10/15/20을 그대로 4배) */
    name: "책임", kind: "active",
    tiers: { 1: 28, 4: 40, 8: 60, 12: 80 },
  ...
  yu_ain: {
    /* 스택당 공격력 상승률 2배(사용자 지침 2026-09-02) → 다시 2배(사용자 지침
     * 2026-09-06 — 파티가 3명이라 스택 상한이 낮아, 배임 등과 맞추려고 올림) */
    name: "보복", kind: "passive",
    tiers: { 1: 2, 4: 4, 8: 6, 12: 8 },
    desc: v => "아군이 죽을 때마다 공격력 " + v + "% 상승 · 자신이 죽으면 초기화"
  },
  lee_hanbeom: {
    /* 스택 하나당 +3%(사용자 지침 2026-09-02 — 원래 +1%에서 3배로)
     * → +6%(사용자 지침 2026-09-06 — 다시 2배로) */
    name: "격노", kind: "passive",
    tiers: { 1: 12, 4: 10, 8: 8, 12: 6 },
    desc: v => "잃은 체력 " + v + "%마다 추가 피해 6%"
  },
  lee_sojeong: {
    /* 스택 하나당 방어 +3(사용자 지침 2026-09-02 — 원래 +1에서 3배로) */
    name: "배임", kind: "passive",
    tiers: { 1: 12, 4: 10, 8: 8, 12: 6 },
    desc: v => "잃은 체력 " + v + "%마다 방어 +3"
  }
};
