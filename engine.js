 ...
 *
 *  ■ 0 으로 내렸습니다 (사용자 지침 2026-09-11)
 *    피해가 «공격 − 방어» 라 방어는 완만하게 올려도 결국 문을 닫습니다.
 *    7장 G안(방어 30)이 들어오면서 익스트림에서 방어가 54가 되었는데,
 *    ★★★ 셋을 어떻게 짜도 한 대에 1~4 밖에 안 들어가 «아예 못 깨는»
 *    자리가 되었습니다. 앞으로도 장이 늘수록 방어가 커질 텐데 그때마다
 *    같은 일이 납니다. 그래서 거울에서는 방어를 아예 안 부풀립니다 —
 *    체력과 공격만 배수를 받습니다. 세진 것이 «단단해서 못 뚫는 것» 이
 *    아니라 «질겨서 오래 걸리는 것» 이 되도록 한다는 원래 뜻 그대로입니다.
 *    이미 나가 있던 적들은 방어가 10~14 라 조금 쉬워지는 정도입니다
 *    (익스트림 타나콘다 17턴 → 14턴).
 */
const DEF_SCALE = 0;
